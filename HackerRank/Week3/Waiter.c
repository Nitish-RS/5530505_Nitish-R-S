...
int* waiter(int number_count, int* number, int q, int* result_count) {
    ...
    int *s = malloc(number_count * sizeof(int));
    int sp = 0;
    for (int i = 0; i < number_count; i++) s[sp++] = number[i];

    for (int i = 0; i < q; i++) {
        int *a = malloc(sp * sizeof(int)), ap = 0;
        int *b = malloc(sp * sizeof(int)), bp = 0;
        for (int j = sp - 1; j >= 0; j--) {
            if (s[j] % pr[i] == 0) a[ap++] = s[j];
            else b[bp++] = s[j];
        }
        for (int j = ap - 1; j >= 0; j--) res[ri++] = a[j];
        free(s);
        s = b;
        sp = bp;
        free(a);
    ...
    for (int j = sp - 1; j >= 0; j--) res[ri++] = s[j];
    *result_count = ri;
    return res;
}
